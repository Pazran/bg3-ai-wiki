"""
BG3 Wiki Vector Database Indexer
=================================
This script ingests the JSON files generated by the scraper and creates a 
ChromaDB vector store for semantic search capabilities (RAG).

Author: Pazran
Dependencies: chromadb, onnxruntime, requests, mwparserfromhell, json, os, re
"""
...
def clean_wiki_syntax(text):
    ...
    Handles specific BG3 wiki template patterns that would otherwise confuse 
    an LLM during semantic search (e.g., {{RarityItem|Name}} -> "Name").
    """
    # 1. Convert {{RarityItem|Whispering Promise}} -> "Whispering Promise"
    # Extracts the content between pipes inside RarityItem templates
    text = re.sub(r'\{\{RarityItem\|([^}]+)\}\}', r'\1', text)
    
    # 2. Convert armor type templates to readable names
    ...
    # Removes orphaned templates that weren't caught by specific regexes above
    text = re.sub(r'\{\{[^}]+\}\}', '', text)
    
    # 4. Clean up any leftover internal wiki links [[Item Name|Display Name]] -> "Display Name"
    # Handles links with explicit display names (e.g., [[Sword of Cursed Flames|Cursed Sword]])
    text = re.sub(r'\[\[[^\]|]+\|([^\]]+)\]\]', r'\1', text)
    # Handles plain links without display names (e.g., [[Item]]) -> "Item"
    text = re.sub(r'\[\[([^\]]+)\]\]', r'\1', text)
...
def get_embedding_function():
    """
    Automatically selects the best available hardware provider for embeddings.
...
for root, dirs, files in os.walk(script_dir):
    # Modifying dirs in-place lets os.walk skip ignored paths entirely (prevents descending into them)
    ...
    if relative_folder == ".":
        continue # Skip files sitting loose in the root directory (not inside a category folder)
    ...
    for filename in files:
        ...
        for line in readable_lines:
            ...
            # Skip SEO and navigation artifacts that add noise to embeddings
            if any(line_strip.startswith(x) for x in ["{{PageSeo", "{{TOC", "{{Companion tab", "[[File:", "| image"]):
                continue
            
            # Remove closing braces from templates (e.g., }}) and empty lines
            if line_strip == "}}" or line_strip == "":
                continue
            
            # --- CLEANING STEP RUNS HERE ---
            cleaned_line = clean_wiki_syntax(line_strip)
            if cleaned_line.strip():  # Only add non-empty, cleaned lines
        ...
        for para in paragraphs:
            ...
            # Skip empty paragraphs or trivial snippets (under 40 characters) to save vector space
            if len(clean_para) < 40:
                continue
                
            # Inject structural cross-referencing metadata into every individual chunk
            # This helps the LLM understand context during retrieval
            structured_chunk = f"DOCUMENT TITLE: {page_title}\n"
            structured_document = f"DOCUMENT TITLE: {page_title}\n"  # Note: 'structured_document' is defined but unused; logic preserved as-is
